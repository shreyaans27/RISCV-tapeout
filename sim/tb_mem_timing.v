...
module tb_mem_timing;
    ...
    // --------------------------------------------------------
    initial begin
        $readmemh("firmware_mem.hex", sram_mem);
    end
    ...
    initial begin
        // VCD dump — all signals for waveform viewing
        $dumpfile("mem_timing.vcd");
        $dumpvars(0, tb_mem_timing);
        ...
        // Let a few more cycles run for clean waveforms
        #200;
    ...
    // --------------------------------------------------------
    // Print key memory operations
    // --------------------------------------------------------
    always @(posedge u_dut.clk_50) begin
        if (rst_n && u_dut.u_core.state == 3'd2) begin // S_EXECUTE
            if (u_dut.u_core.opcode == 7'b0000011)  // LOAD
                $display("[cycle %0d] LOAD  addr=0x%08x (PC=0x%08x)",
                    cycle_count, u_dut.u_core.alu_result, u_dut.u_core.pc);
            if (u_dut.u_core.opcode == 7'b0100011)  // STORE
                $display("[cycle %0d] STORE addr=0x%08x data=0x%08x (PC=0x%08x)",
                    cycle_count, u_dut.u_core.alu_result,
                    u_dut.u_core.rs2_data, u_dut.u_core.pc);
        end
    end
