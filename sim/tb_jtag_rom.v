...
module tb_jtag_rom;
    ...
    endtask
    task jtag_reset;
        begin jtag_tms = 1; repeat(5) jtag_clock; jtag_tms = 0; jtag_clock; end
    endtask
    task goto_shift_ir;
    ...
    task write_ir;
        ...
        begin
            ...
            for (i = 0; i < 3; i = i + 1) begin
                jtag_tdi = val[i]; jtag_tms = (i == 2) ? 1 : 0; jtag_clock;
            end
            goto_rti;
        end
    endtask
    task shift_dr_68;
        ...
        begin
            ...
            for (i = 0; i < 68; i = i + 1) begin
                jtag_tdi = din[i]; jtag_tms = (i == 67) ? 1 : 0; jtag_clock;
                dout[i] = jtag_tdo;
            end
            goto_rti;
        end
    ...
    initial begin
        $dumpfile("jtag_rom_debug.vcd");
        $dumpvars(0, tb_jtag_rom);
        ...
        // Read ROM at 0x20000004 (WL=1, COL=0 → expect 0x55555555)
        $display("--- Sending ROM read command ---");
        shift_dr_68({2'b00, 32'h20000004, 32'h0, 2'b00}, resp);
        
        // Monitor ROM controller state
        repeat(50) begin
            @(posedge u_dut.clk_25);
            $display("  rom_ctrl: state=%0d preen=%b wlen=%b saen=%b wl=%02x col=%01x dout=0x%08x | bus: req_v=%b resp_v=%b last_rd=0x%08x bus_st=%0d",
                u_dut.u_rom.state,
                rom_preen, rom_wlen, rom_saen,
                rom_wl_addr, rom_col_in, rom_dout,
                u_dut.u_jtag.req_valid, u_dut.u_jtag.resp_valid,
                u_dut.u_jtag.last_rdata,
                u_dut.u_jtag.bus_state);
        end

        // Capture result
        shift_dr_68({2'b11, 32'h0, 32'h0, 2'b00}, resp);
        $display("\nRead back: status=%0b rdata=0x%08x", resp[67:66], resp[65:34]);
