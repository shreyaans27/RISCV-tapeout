...
# -----------------------------------------------------------------------------
# Ground-truth warning list from Innovus probe (innovus.log5)
# -----------------------------------------------------------------------------

MISSING_GATEAREA = [
    # (cell_name, output_pin_name) -- pin needs ANTENNAGATEAREA added
    ("inv_2",    "Y"),
    ("inv_4",    "Y"),
    ("inv_8",    "Y"),
    ("mux2",     "X"),
...
MISSING_DIFFAREA = [
    # (cell_name, input_pin_name) -- pin needs ANTENNADIFFAREA added
    ("nor2_4",  "A"),  ("nor2_4",  "B"),
    ("nor2_8",  "A"),  ("nor2_8",  "B"),
    ("o21ai_1", "A1"), ("o21ai_1", "A2"), ("o21ai_1", "B1"),
    ("o21ai_2", "A1"), ("o21ai_2", "A2"), ("o21ai_2", "B1"),
    ("o21ai_4", "A1"), ("o21ai_4", "A2"), ("o21ai_4", "B1"),
    ("o21ai_8", "A1"), ("o21ai_8", "A2"), ("o21ai_8", "B1"),
    ("xnor2_1", "A"),  ("xnor2_1", "B"),
    ("xor2_1",  "A"),  ("xor2_1",  "B"),
]
...
def build_patch_plan(cells, all_lines):
    """
    For each (cell, pin) in MISSING_GATEAREA/MISSING_DIFFAREA, work out:
      - insertion line (just before the PIN's PORT statement)
      - lines to insert (exactly as extracted from donor pin)
    ...
    # --- GATEAREA insertions on output pins ---
    for (cell, out_pin) in MISSING_GATEAREA:
        if cell not in cells:
            errors.append((cell, out_pin, "cell not found in LEF"))
            continue
        cinfo = cells[cell]
        if out_pin not in cinfo["pins"]:
            errors.append((cell, out_pin, f"pin '{out_pin}' not in cell"))
            continue
        target_pin = cinfo["pins"][out_pin]
        if target_pin["first_port_line"] is None:
            errors.append((cell, out_pin, "no PORT statement found in pin"))
            continue

        donor_pin_name = find_donor_input_pin(cinfo)
        if donor_pin_name is None:
            errors.append((cell, out_pin, "no input pin with GATEAREA to copy from"))
            continue
        donor_pin = cinfo["pins"][donor_pin_name]

        model_line, gate_lines = extract_gatearea_block_from_pin(donor_pin, all_lines)
        if not gate_lines:
            errors.append((cell, out_pin, f"donor pin '{donor_pin_name}' has no GATEAREA after all"))
            continue

        # Build insertion block: ANTENNAMODEL scope + GATEAREA lines
        # Indentation in this LEF: PIN attrs at 4 spaces, model-scoped at 6 spaces
        lines_to_insert = []
        if model_line is not None:
            lines_to_insert.append(model_line)
            lines_to_insert.extend(gate_lines)
        else:
            # No model wrapper seen; emit GATEAREA as top-level pin attrs
            lines_to_insert.extend(gate_lines)

        plan.append({
            "cell":            cell,
            "target_pin":      out_pin,
            "insert_at_line":  target_pin["first_port_line"],
            "lines_to_insert": lines_to_insert,
            "source":          f"{cell}.{donor_pin_name}",
            "kind":            "GATEAREA",
        })

    # --- DIFFAREA insertions on input pins ---
    for (cell, in_pin) in MISSING_DIFFAREA:
        if cell not in cells:
            errors.append((cell, in_pin, "cell not found in LEF"))
            continue
        cinfo = cells[cell]
        if in_pin not in cinfo["pins"]:
            errors.append((cell, in_pin, f"pin '{in_pin}' not in cell"))
            continue
        target_pin = cinfo["pins"][in_pin]
        if target_pin["first_port_line"] is None:
            errors.append((cell, in_pin, "no PORT statement found in pin"))
            continue

        donor_pin_name = find_donor_output_pin(cinfo)
        if donor_pin_name is None:
            errors.append((cell, in_pin, "no output pin with DIFFAREA to copy from"))
            continue
        donor_pin = cinfo["pins"][donor_pin_name]
        diff_lines = extract_diffarea_lines_from_pin(donor_pin)
        if not diff_lines:
            errors.append((cell, in_pin, f"donor pin '{donor_pin_name}' has no DIFFAREA"))
            continue
